...
可变方法能够赋给隐含属性`self一个全新的实例。上面Point的例子可以用下面的方式改写：`
...
实例方法是被某个类型的实例调用的方法。你也可以定义在类型本身上调用的方法，这种方法就叫做_类型方法_。在方法的`func关键字之前加上关键字static，来指定类型方法。类还可以用关键字class来允许子类重写父类的方法实现。`
